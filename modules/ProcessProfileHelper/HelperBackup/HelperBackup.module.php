...
class HelperBackup extends ProcessProfileHelper {
	...
	public function __construct() {
		...
		$this->set('backupFolderPAth', config()->paths->site . 'assets/backups/site-helper');
		// $this->remove('backupFilesPAth');
	...
	public function backupSite(HookEvent $e) {
		...
		if ($this->countBackupFiles >= $this->limitBackupFiles) {
			...
			// Find all files
			$getFiles = files()->find(config()->paths->site . 'assets/backups/site-helper', ['extensions' => $extensions]);
	...
	public function processCustomDbConfig($zipBackupFile = '') {
		...
		$modulePAth = config()->paths->site . 'modules/ProcessProfileHelper/' . substr(strrchr(__CLASS__, "\\"), 1) . '/';
		...
		$customConfig = files()->fileGetContents($modulePAth . 'inc/config-dev.php');
		$customConfig = str_replace(
			...
			$zip->addFile($path . 'config.php', $firstFolderName . 'site/config.php');
			$zip->close();
			// Set success message
